...
- 多级目录：对文件系统进行了修改，使得文件系统变成了多级的文件系统。可以进行通过cd进出文件夹、通过ls参看文件夹内容、通过touch新建文件等7项操作。
...
输出一张9*9的地图，player1和player2交替输入落子坐标来进行玩家对战，player1的落子显示为‘X’，player2的落子显示为‘O’。任何一方实现五子连线都将显示该方获胜并退出。按‘e’可以直接退出游戏。
